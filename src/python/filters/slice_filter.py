...
class SliceFilter(FilterBase):
    ...
    def create_tool(
        ...
        try:
            ...
            return "Error: Invalid offsets format. Use comma-separated numbers."
        ...
        if result:
            ...
            return f"Applied slice filter to '{target_item.name}'. New item: '{result.name}' (id: {result.id})"
    ...
    def update_tool(
        ...
        if not item or item.item_type != "slice_filter":
            return f"Error: Item {target_id} is not a slice filter"
        ...
        if offsets is not None:
            try:
                params["offsets"] = [float(x.strip()) for x in offsets.split(",")]
                updated.append(f"offsets={offsets}")
            except ValueError:
                return "Error: Invalid offsets format."
        ...
        if apply:
            ...
            return f"Updated and applied slice filter: {', '.join(updated)}"
