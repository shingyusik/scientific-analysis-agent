...
class SliceFilter(FilterBase):
    ...
    @expose_filter_tool(
        ...
        params_model=SliceParams,
        update_description=(
            "Updates the parameters of an existing Slice filter.\n"
            "Parameters:\n"
            "- 'offsets': List of floats (e.g. '0.0, 5.0') to change the number and position of slices relative to the origin.\n"
            "- 'normal_x', 'normal_y', 'normal_z': Update the normal vector of the slice plane.\n"
            "- 'origin_x', 'origin_y', 'origin_z': Update the point on the plane.\n"
            "- 'show_plane': Toggle the plane widget visibility.\n"
            "- 'item_id': The ID of the slice filter item to update. REQUIRED."
        )
    )
